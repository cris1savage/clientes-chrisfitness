...
export default function MesClient({ clienteId, clienteName, phases, initialCheckins }) {
  ...
  const weeklyNotes = useMemo(() => {
    ...
    const saved = checkin.weekly_notes || [];
    return realWeeks.map((w, i) => {
      // Buscar datos guardados — primero por weekStart exacto, luego por índice
      // PERO las fechas (weekStart, weekEnd, label) SIEMPRE vienen del calendario real
      const existing = saved.find((n) => n.weekStart === w.weekStart) || saved[i] || {};
      return {
        weekStart: w.weekStart,   // SIEMPRE del calendario real
        weekEnd:   w.weekEnd,     // SIEMPRE del calendario real
        label:     w.label,       // SIEMPRE del calendario real
